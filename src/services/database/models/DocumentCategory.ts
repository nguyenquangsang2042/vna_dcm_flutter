...
export class DocumentCategory extends Model {
    static table = 'document_category';
    // @ts-ignore
    ...
    // @ts-ignore
    static async getAll(AreaCategoryId,limit, offset) {
        // @ts-ignore
        const table = database.get<DocumentCategory>(DocumentCategory.table);
        const data: DocumentCategory[] = await table.query(
            Q.where('AreaCategoryId',AreaCategoryId),
