...
export default class DocumentRecently extends Model {
    static table = 'document_recently';
    // @ts-ignore
    ...
    {
        // @ts-ignore
        const table = database.get<DocumentRecently>(DocumentRecently.table);
        ...
        database.write(async () => {
            for (const model of models) {
                ...
                if (data?.length > 0) {
                    ...
                    if (__DEV__)
                        console.log('update DocumentRecently:', model);
                } else {
                    if (__DEV__)
                        console.log('insert DocumentRecently:', model);
                    // @ts-ignore
                    table.create(field => {
